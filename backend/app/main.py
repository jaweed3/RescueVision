...
async def serve_tile(z: int, x: int, y: int):
    """Proxy tile from OSM with disk cache — enables offline map after first load."""
    ...
    # Fetch from OSM, rotate servers
    server = TILE_SERVERS[hashlib.md5(cache_key.encode()).digest()[0] % len(TILE_SERVERS)]
    ...
    try:
        ...
        logger.warning("[TILE] Fetch failed for %s: %s", url, e)
...
async def export_json():
    """Export all detected victim coordinates as JSON."""
    return JSONResponse(content=list(DETECTIONS_LOG))
