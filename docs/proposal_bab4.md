...
| Frontend | React + Vite (SPA) | Modern UI, canvas overlay, Leaflet.js map integration |
...
POST /detect
  ...
  Output: {
    ...
    "gps_source": "exif",
    "ref_coords": {"lat": -7.3418, "lon": 110.4521, "altitude": 80.0}
  }
...
- Peta interaktif (Leaflet.js dengan tile caching lokal)
- Tombol *export* koordinat ke CSV/JSON untuk dimasukkan ke GPS handheld
...
tidak pernah meninggalkan perangkat operator. Log sesi inferensi
disimpan dalam memori (in-memory) dan dapat dihapus kapan saja
melalui tombol Clear Session. Fitur auto-clear pada session timeout
tersedia secara opsional. Koordinat korban yang diekspor dalam format
CSV/JSON hanya dapat diakses oleh koordinator yang menjalankan sistem,
